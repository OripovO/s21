...
def generate_step(grid, frontier, seen, current_node):
    ...
    # pick one of the successors (or from frontier if no successors)
    if len(successors) > 0:
        current_node = pick_node_from_list(successors)
    else:
        current_node = pick_node_from_list(frontier) if frontier else Node(None, 0, (0,0))
